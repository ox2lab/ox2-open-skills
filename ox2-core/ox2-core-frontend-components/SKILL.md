...
기본 산출물 구조는 책임별로 나눈다. 페이지 템플릿은 레이아웃과 컴포넌트 배치를 담당하고, 페이지 컨트롤러는 데이터 로딩, API 호출, 화면 흐름, 컴포넌트 이벤트 연결을 담당한다. 컴포넌트는 독립 UI 책임, 내부 상태, 렌더링, 사용자 조작 이벤트만 가진다. 모델과 프록시는 API 호출, 응답 코드 판단, 응답 데이터 처리를 담당한다.
...
3. 새 UI 또는 큰 수정이라면 구현 전에 컴포넌트 분해안을 작성했는지 확인한다.
