...
프론트엔드 UI 작업은 컴포넌트 우선 설계를 기본값으로 삼는다. 사용자가 단순 마크업 수정, 빠른 임시 수정, 기존 구조 유지처럼 범위를 명확히 제한하지 않았다면 화면 전체를 긴 템플릿이나 페이지 스크립트에 직접 쌓기보다, 먼저 어떤 화면 책임을 독립 컴포넌트로 둘지 결정한다.
...
3. 상태, 이벤트, 렌더링, 사용자 작업 단위가 있는 UI를 페이지에 직접 쌓지 않았는지 확인한다.
4. 과분리 또는 과집중이 없는지, 컴포넌트를 제거하거나 교체해도 상위 화면의 조립 구조가 유지되는지 확인한다.
5. LazyLoader가 필요한 태그명, prefix, 파일명, `customElements.define()` 이름을 맞춘다.
6. 모델은 API 호출 중심으로 유지하고, 응답 처리와 UI 피드백은 `ModelProxy`와 UI 계층으로 분리한다.
7. 폼 저장/복원은 `FormManager`의 저장키, dataset, change 이벤트 복원 방식을 따른다.
8. 명령형 UI 동작은 `command-` 클래스와 이벤트 위임으로 묶는다.
9. Tview/UIManager는 실제 존재를 확인한 뒤 사용하고, 없으면 주변 코드의 기존 대체 패턴을 따른다.
10. CSS는 페이지별 파일에 필요한 스타일만 추가하고, 가상 요소와 단순 선택자로 불필요한 마크업을 줄인다.
