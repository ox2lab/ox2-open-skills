...
description: ox2-core 프론트엔드 UI, 웹 컴포넌트, 페이지 스크립트, 폼 처리, 모델 호출, 커스텀 엘리먼트, LazyLoader 기반 모듈 로딩을 작성하거나 검토할 때 사용한다. 사용자가 ox2-core 프론트엔드, 컴포넌트 분해안, 웹 컴포넌트, HTMLElement, wdgt-/cmp- 태그, ModelProxy, FormManager, command 이벤트 패턴을 언급하면 반드시 이 스킬을 사용한다. 단순 한 줄 수정이 아닌 기능성 UI 작업에서는 먼저 컴포넌트 우선 설계와 페이지 조립 책임을 정리하고, TView나 UIManager 같은 보조 패턴은 대상 프로젝트의 실제 런타임 존재를 확인한 뒤 적용한다.
---
...
프론트엔드 UI 작업은 컴포넌트 우선 설계를 기본값으로 삼는다. 여기서 컴포넌트는 일반적인 UI 조각이나 외부 프레임워크 컴포넌트가 아니라, ox2-core 웹 개발 런타임에서 쓰는 클래스 기반 웹 컴포넌트와 그에 준하는 독립 UI 책임 단위를 뜻한다. 사용자가 단순 마크업 수정, 빠른 임시 수정, 기존 구조 유지처럼 범위를 명확히 제한하지 않았다면 화면 전체를 긴 템플릿이나 페이지 스크립트에 직접 쌓기보다, 먼저 어떤 화면 책임을 독립 컴포넌트로 둘지 결정한다.
...
2. 어떤 UI를 컴포넌트로 만들었는지, 각 컴포넌트의 책임이 한 문장으로 설명되는지 확인한다.
3. 새 UI 또는 큰 수정이라면 구현 전에 컴포넌트 분해안을 작성했고, 페이지 역할, 컴포넌트 입력값, 발생 이벤트, 상위 조립 책임, 컴포넌트에 넣지 않을 책임을 모두 정리했는지 확인한다.
4. 페이지 템플릿, 페이지 컨트롤러, 컴포넌트, 모델/프록시의 책임이 서로 섞이지 않았는지 확인한다.
5. 상태, 이벤트, 렌더링, 사용자 작업 단위가 있는 UI를 페이지에 직접 쌓지 않았는지 확인한다.
6. 과분리 또는 과집중이 없는지, 컴포넌트를 제거하거나 교체해도 상위 화면의 조립 구조가 유지되는지 확인한다.
7. LazyLoader가 필요한 태그명, prefix, 파일명, `customElements.define()` 이름을 맞춘다.
8. 모델은 API 호출 중심으로 유지하고, 응답 처리와 UI 피드백은 `ModelProxy`와 UI 계층으로 분리한다.
9. 폼 저장/복원은 `FormManager`의 저장키, dataset, change 이벤트 복원 방식을 따른다.
10. 명령형 UI 동작은 `command-` 클래스와 이벤트 위임으로 묶는다.
11. Tview/UIManager는 실제 존재를 확인한 뒤 사용하고, 없으면 주변 코드의 기존 대체 패턴을 따른다.
12. CSS는 페이지별 파일에 필요한 스타일만 추가하고, 가상 요소와 단순 선택자로 불필요한 마크업을 줄인다.
